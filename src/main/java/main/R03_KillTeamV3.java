...
public class R03_KillTeamV3 {
   ...
   public static void move(int round, int move, Player attacker, Player defender) {
      ...
      System.out.println("*----------------*");
      System.out
            .println("Kämpfer von " + attacker.getName() + ": " + attacker.getFighter().getName()
                  + " (" + attacker.getFighter().getHealthPoints() + " LP)");
      System.out
            .println("Kämpfer von " + defender.getName() + ": " + defender.getFighter().getName()
      ...
      if (totalHits > totalBlocks) {
         int damage =
               (totalHits - totalBlocks) * attacker.getFighter().getWeapon().getDamageValue();
